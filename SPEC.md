...
- `src/agent.mjs` — **cc-alt** harness: system prompt teaching the compact-edit protocol + `edit_file`.
- `src/baseline.mjs` — **baseline** harness: same loop/tools, system prompt teaching read-whole →
  write-whole + `write_file`. The ONLY difference from cc-alt is the edit/context protocol.
