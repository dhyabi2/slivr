<p align="center"><img src="assets/logo.png" width="116" alt="Proov logo"></p>

<h1 align="center">Proov</h1>
...
GPT, Gemini, …) over OpenRouter. Proov's edge is its **edit engine**: a precise anchor-based protocol
...
```bash
# one line — puts `proov` on your PATH (pure Node >= 18, no build, no deps)
curl -fsSL https://raw.githubusercontent.com/dhyabi2/proov/main/install.sh | bash

# …or run without installing
npx github:dhyabi2/proov --help

# …or from a clone
git clone https://github.com/dhyabi2/proov && cd proov && npm link
```

> **Pin a version.** The one-liner tracks `main` by default. To install a specific tag/commit, set
> `REF`: `REF=v1.2.3 curl -fsSL https://raw.githubusercontent.com/dhyabi2/proov/main/install.sh | bash`.
...
```bash
proov upgrade            # fast-forward the install to the latest release
proov upgrade --check    # just tell me if a newer version exists (no changes)
```
...
<p align="center">
  <img src="assets/comparison.svg" width="940" alt="Coding-agent feature comparison: Proov vs Claude Code, Aider, OpenHands, Cursor — terminal, any-model, BYO-key, precise edits, lowest large-file edit cost, MCP, multimodal, plan-mode, orchestration, background jobs.">
</p>

Proov's distinctive combination: **provider-agnostic + bring-your-own-key + the lowest measured
...
*smarter* than any of these — quality comes from the model you plug in; Proov competes on edit cost,
...
proov --init                    # writes a starter ./.proov.json
proov config                    # show the resolved config and where each value came from

# 4. work
proov                                                  # interactive REPL in the current repo
proov "add input validation to src/calc.js"            # one-shot in the current dir
proov "fix the failing test" ./myrepo --auto           # one-shot, no approval prompts
proov --model anthropic/claude-sonnet-4                # REPL on Claude
```

### The REPL
Running `proov` with no task opens a multi-turn session. **Conversation + tool results persist
...
### Config (`proov config`, `--init`)
Resolved with precedence **flags > `./.proov.json` > `~/.proov.json` > env > defaults**. Keys:
...
| `PROOV_API_KEY` | `apiKey` | alternative key var; **overrides** `OPENROUTER_API_KEY` if both set |
| `MODEL` | `model` | back-compat with the original benchmark CLI |
| `PROOV_MODEL` | `model` | **overrides** `MODEL` if both set |
| `PROOV_BASE_URL` | `baseUrl` | default `https://openrouter.ai/api/v1` |
| `PROOV_APPROVAL` | `approval` | one of `auto` \| `edits` \| `all` |
| `PROOV_MAX_STEPS` | `maxSteps` | integer (default 16) |
| `PROOV_MAX_TOKENS` | `maxTokensPerTurn` | integer (default 4000) |

### Code navigation (`find_symbol` / `repo_map`)
proov builds a zero-dependency **symbol index** of the repo (regex scan; no vector DB) so the agent
can **jump to a definition** instead of grepping through every mention: `find_symbol <name>` →
`file:line` + signature; `repo_map` → a compact map of files and their top-level symbols. On proov's
...
Give proov a check and it won't finish until the check passes. When the agent calls `done`, proov runs
...
```bash
proov "make the failing tests pass" --auto --verify "npm test"
proov "fix the type errors" --auto --verify "tsc --noEmit" --repair 5
```
...
### CLI reference
```
proov                       interactive REPL in the current directory
proov "<task>" [dir]        one-shot
proov config                print resolved config
proov --init                write a starter ./.proov.json
proov mcp list              connect configured MCP servers and list their tools
proov mcp add <name> -- <command...>   add an MCP server to ./.proov.json
proov skills                list skills    ·   proov skill <name> [args]   run one
proov bg "<task>" [dir]     detached background run (POSIX)  ·  jobs / logs <id>
proov schedule "<task>" --in 30m|--at <ISO>|--cron "<expr>"   ·  schedule list / clear
proov scheduler [--daemon|status|stop] [--every <sec>]   run/manage the schedule poller
proov upgrade [--check]     update the install to the latest version (forward-only, dirty-safe)
...
```
proov --plan "refactor the auth module"        # interactive: review the plan, then it executes
proov --plan --auto "add a healthcheck route"  # shows the plan, auto-approves, executes
...
proov is an **MCP (Model Context Protocol) client** (stdio transport), the same extensibility
...
Configure servers in `./.proov.json` (or `~/.proov.json`) under an `mcpServers` block — the same
...
Then:
```
proov mcp list                                       # connect + print discovered tools per server
proov mcp add weather -- npx -y some-weather-mcp     # write a server entry into ./.proov.json
proov "use mcp__everything__echo to echo 'hi'"       # the model calls the namespaced tool
```
...
the rest. If no `mcpServers` are configured, nothing changes and proov behaves exactly as before.

### Multimodal — let the model SEE images and READ PDFs

proov can attach images and PDFs to the conversation so a vision model actually looks at them. Two
...
```
proov "view_image screenshot.png and describe the error dialog" --auto
proov "view_pdf spec.pdf and summarize section 3" --auto
```
Verified live: the agent read the word and colors in a generated PNG, and extracted text from a PDF.

**PDF text extraction (with automatic local fallback).** For text PDFs proov first tries OpenRouter's
...
A skill is a Markdown file = a saved prompt you can run by name. Discovery order: **`./.proov/skills/*.md`
(project)** then **`~/.proov/skills/*.md` (user)**; a project skill shadows a user skill of the same
...
```
proov skills                       # list discovered skills (name + description)
proov skill review                 # run a skill one-shot
proov skill commit "context here"  # args fill $ARGS / $1 …
```
In the REPL:
```
/skills              list skills
...
```
Ships with three examples under [`.proov/skills/`](.proov/skills/): `review.md` (review the staged
diff), `test.md` (find + run the suite, fix failures), `commit.md` (write a message + commit). Drop
your own `.md` files in `./.proov/skills/` to add more — no code changes.

Example skill (`.proov/skills/review.md`):
```markdown
# Review staged diff
...
for later. State lives under `~/.proov/` (`jobs/<id>.json` + `jobs/<id>.log`, and `schedule.json`).

```
proov bg "fix the failing test and commit" ./repo   # detached; runs proov one-shot --auto
proov jobs                                           # list jobs: id, status (queued/running/done/failed), task
proov jobs --watch                                   # repaint every 2s
proov logs <id>                                      # print that job's captured log

proov schedule "run the nightly check" --in 2h       # also: --at <ISO>  or  --cron "*/5 * * * *"
proov schedule list                                  # scheduled jobs, GROUPED active vs done, + next run
...
proov scheduler                                      # foreground sleep-loop poller (Ctrl-C to stop)
proov scheduler --daemon                             # detach + write ~/.proov/scheduler.pid
proov scheduler status                               # report running / not (reads the pidfile)
proov scheduler stop                                 # stop the detached poller
```
**Honest about the design:** `proov bg` spawns a real detached child (`spawn(..., {detached:true,
stdio:'ignore'})`) on **macOS/Linux**; on Windows (or if the detached spawn fails) it refuses with a
clear message rather than crashing — background tasks need a POSIX shell. `proov scheduler` is a
...
`proov bg`; once-jobs are marked `done` (clear them with `schedule clear`), cron-jobs are rescheduled
...
| | proov | baseline (Claude-Code-style) |
|---|---|---|
...
- **proov** failed `config-constant`: the model emitted **syntactically invalid code**
  (`export import {...}`) — a *model* error, not a harness one (proov made the edits cleanly in 3
  turns with 0 edit failures). An earlier version of proov failed this task with 15 edit failures
...
failed the oracle. proov read once, made one targeted edit, verified, and finished: **99.3%
cheaper AND higher success**. This is the clearest expression of the thesis.

### Where the win is marginal or NEGATIVE (honest)
On a **tiny single-edit file** (`fix-offbyone`, 7 lines) proov was **~73% MORE expensive**
...
- skills: `src/skills.mjs` (discovery + arg substitution) · `.proov/skills/*.md` (example prompts) · user
  skills in `~/.proov/skills/` (e.g. klokwork 3D engine, vgsds 3D-asset MCP, level/multiplayer patterns)
...
- `bin/proov.mjs` — main CLI · `bin/agent.mjs` — original benchmark CLI · `demo.mjs` — live side-by-side · `selftest.mjs` — deterministic (no LLM)
- `bench/tasks.mjs` `bench/run.mjs` `bench/results.json` · `SPEC.md`

## Run it
```
node selftest.mjs                                   # 145 deterministic tests, no API key
proov                                              # REPL (after `npm link`)
node bin/proov.mjs "<task>" ./repo --auto          # one-shot without install
...
**[GNU Affero General Public License v3.0 (AGPL-3.0)](LICENSE)** — Proov is **free and open source**.
